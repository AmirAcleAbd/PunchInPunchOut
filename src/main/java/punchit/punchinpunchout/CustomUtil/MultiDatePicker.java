...
public class MultiDatePicker
{
    ...
    public MultiDatePicker(DatePicker rangePicker){
        this.selectedDates = FXCollections.observableSet(new TreeSet<>());
    ...
    public LocalDate getLowerValue() {
        if (!selectedDates.isEmpty()) {
            return selectedDates.stream().min(LocalDate::compareTo).orElse(null);
        }
        return null;
    }

    public LocalDate getUpperValue() {
        if (!selectedDates.isEmpty()) {
            return selectedDates.stream().max(LocalDate::compareTo).orElse(null);
        }
        return null;
    }
}